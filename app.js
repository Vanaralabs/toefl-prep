...
function speakSentence(text) {
  if (!('speechSynthesis' in window)) {
    alert('⚠️ Your browser does not support text-to-speech. Use Chrome for audio.');
    return false;
  }
  ...
  // Get voices (may need retry on first load)
  let voices = window.speechSynthesis.getVoices();
  if (voices.length === 0) {
    // Chrome loads voices async — try again after short delay
    setTimeout(() => {
...
// ========== LISTEN & REPEAT RECORDER ==========
let repeatRecognition = null;
let repeatRecActive = false;

function toggleRepeatRecording() {
  if (repeatRecActive) {
    stopRepeatRecording();
  } else {
...
function startRepeatRecording() {
  navigator.mediaDevices.getUserMedia({ audio: true }).then(stream => {
    repeatRecActive = true;
    
    document.getElementById('repeatRecBtn').innerHTML = '<i class="fa-solid fa-circle" style="color:#fff"></i> Recording...';
    document.getElementById('repeatRecBtn').style.background = '#dc2626';
    document.getElementById('repeatStopBtn').disabled = false;
    document.getElementById('repeatRecStatus').textContent = '🔴 Speak now...';
    document.getElementById('repeatRecStatus').className = 'recorder-status recording';
    document.getElementById('repeatInput').value = '';
    
    const SpeechRecognition = window.SpeechRecognition || window.webkitSpeechRecognition;
    if (!SpeechRecognition) {
      document.getElementById('repeatRecStatus').textContent = '⚠️ Speech recognition not supported. Use Chrome. Type manually.';
      stream.getTracks().forEach(t => t.stop());
      resetRepeatRecUI();
      return;
    }
    
    repeatRecognition = new SpeechRecognition();
    repeatRecognition.continuous = true;
    repeatRecognition.interimResults = true;
    ...
    repeatRecognition.onresult = (event) => {
      ...
      document.getElementById('repeatInput').value = (finalTranscript + interim).trim();
    };
    ...
    repeatRecognition.onend = () => {
      if (repeatRecActive) repeatRecognition.start();
    };
    
    repeatRecognition.start();
    
    // Store stream to stop later
    repeatRecognition._stream = stream;
    
  }).catch(err => {
    alert('🎤 Mic blocked! Click lock icon in URL bar → Allow Microphone → Reload');
  });
}

function stopRepeatRecording() {
  repeatRecActive = false;
  if (repeatRecognition) {
    repeatRecognition.onend = null;
    repeatRecognition.stop();
    if (repeatRecognition._stream) repeatRecognition._stream.getTracks().forEach(t => t.stop());
  }
  resetRepeatRecUI();
  document.getElementById('repeatRecStatus').textContent = 'Done ✅ — Click Check to compare';
}

function resetRepeatRecUI() {
  document.getElementById('repeatRecBtn').innerHTML = '<i class="fa-solid fa-microphone"></i> Record Your Repeat';
  document.getElementById('repeatRecBtn').style.background = '';
  document.getElementById('repeatStopBtn').disabled = true;
  document.getElementById('repeatRecStatus').className = '';
}
...
function startRecording() {
  ...
  navigator.mediaDevices.getUserMedia({ audio: true }).then(stream => {
    ...
    document.getElementById('recorderStatus').textContent = '❌ Mic blocked — allow permission above';
    console.error(err);
  });
}
