    ...
    area.innerHTML = `
      <div class="mock-question">
        ${section === 'reading' ? `<div class="q-passage">${passage.passage}</div>` : 
          `<div class="q-passage"><strong>${passage.context}</strong><br><br>"${passage.transcript}"</div>`}
...
function showMockResults() {
  ...
  if (results.reading) {
    html += `<div class="result-section"><h4><i class="fa-solid fa-book-open"></i> Reading</h4>
      ...
      <div class="result-details">Estimated: ${results.reading.scaled}/30</div></div>`;
  }
  if (results.listening) {
    html += `<div class="result-section"><h4><i class="fa-solid fa-headphones"></i> Listening</h4>
      ...
      <div class="result-details">Estimated: ${results.listening.scaled}/30</div></div>`;
  }
  if (results.speaking) {
...
function evaluateMockTest() {
  ...
  if (mockState.sections.includes('reading')) {
    ...
    let correct = 0;
    allQs.forEach((q, i) => { if (mockState.answers[`reading_${i}`] === q.answer) correct++; });
    const scaled = Math.round((correct / allQs.length) * 30);
    results.reading = { correct, total: allQs.length, scaled };
  ...
  if (mockState.sections.includes('listening')) {
    ...
    const allQs = data.slice(start, start + 2).reduce((acc, p) => acc.concat(p.questions), []);
    let correct = 0;
    allQs.forEach((q, i) => { if (mockState.answers[`listening_${i}`] === q.answer) correct++; });
    const scaled = Math.round((correct / allQs.length) * 30);
    results.listening = { correct, total: allQs.length, scaled };
    totalEstimated += scaled; sectionCount++;
  }
